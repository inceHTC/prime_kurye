...
async function getBusinessReport(req, res) {
    try {
        ...
        const activeOrders = orders.filter(o => ['PENDING', 'CONFIRMED', 'PICKING_UP', 'IN_TRANSIT'].includes(o.status));
...
async function getAdminReport(req, res) {
    try {
        ...
        deliveredOrders.forEach(order => {
            const day = order.createdAt.toISOString().split('T')[0];
            if (!dailyMap[day])
                dailyMap[day] = { orders: 0, revenue: 0, commission: 0 };
            dailyMap[day].orders++;
        ...
        const topCouriers = await Promise.all(Object.entries(courierMap)
