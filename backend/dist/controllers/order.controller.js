...
exports.autoConfirmDeliveries = void 0;
...
function calculatePrice(distance, deliveryType, isFragile = false, packageValue = 0) {
    const basePrices = {
        EXPRESS: 136.9,
        SAME_DAY: 94.9,
        SCHEDULED: 110.9,
...
async function createOrder(req, res) {
    try {
        ...
        const business = user.business ?? await prisma_1.prisma.business.create({
            data: {
                userId,
                companyName: user.fullName,
            },
        });
        const { senderName, senderPhone, senderAddress, senderLat, senderLng, senderNotes, recipientName, recipientPhone, recipientAddress, recipientLat, recipientLng, recipientNotes, packageDesc, packageWeight, isFragile, packageValue, deliveryType, insuranceValue, scheduledAt, } = req.body;
        const distance = calculateDistance(senderLat, senderLng, recipientLat, recipientLng);
        ...
        const nearestCouriers = await (0, courierMatching_service_1.findNearestCouriers)(parseFloat(senderLat), parseFloat(senderLng), 3);
...
const autoConfirmDeliveries = async () => {
    try {
